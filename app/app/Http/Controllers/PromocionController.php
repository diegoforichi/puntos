...
class PromocionController extends Controller
{
    ...
    public function notificarClientes(Request $request, $tenantRut, $id)
    {
        $tenant = $request->attributes->get('tenant');
        $usuario = $request->attributes->get('usuario');

        if (! in_array($usuario->rol, ['admin', 'supervisor'])) {
            return back()->with('error', 'No tiene permisos para notificar clientes');
        }
        ...
        if (! ($eventos['promociones_activas'] ?? false)) {
            return back()->with('warning', 'El evento "Promociones Activas" está desactivado en la configuración de WhatsApp');
        }
